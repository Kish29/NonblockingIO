...
class epoll_wrapper {
public:
    typedef std::function<void(const struct epoll_event &)> epoll_event_callback;
    ...
    int add_event(int fd, int event, epoll_event_callback callback);
...
private:
    ...
    std::map<int, epoll_event_callback> callbacks_{};
